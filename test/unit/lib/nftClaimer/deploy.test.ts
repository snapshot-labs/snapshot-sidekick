...
const mockValidateSpace = jest.fn(
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  (address: string, space: Space | null): any => {
    return true;
  }
);
// eslint-disable-next-line @typescript-eslint/no-unused-vars
...
jest.mock('../../../../src/lib/nftClaimer/utils', () => {
  ...
  return {
    __esModule: true,
    ...originalModule,
    validateSpace: (address: string, space: Space | null) =>
      mockValidateSpace(address, space),
...
describe('nftClaimer', () => {
  describe('payload()', () => {
    ...
    const salt =
      '72536493147621360896130495100276306361343381736075662552878320684807833746288';
    ...
    const expectedDigest =
      '0xc3a4ff2bd44ee17fbc4f802953bd55392238b034c71ca4f5de2ad2f23fa31af7';
    ...
    describe('when passing invalid values', () => {
      it.skip('throws an error when the spaceOwer address is not valid', () => {
        expect(
          async () => await getPayload({ spaceOwner: 'test' })
        ).rejects.toThrow();
      });

      it.skip('throws an error when the spaceTreasury address is not valid', () => {
        expect(
          async () => await getPayload({ spaceTreasury: 'test' })
        ).rejects.toThrow();
      });
      ...
      it.each(NAN)(
        ...
        val => {
          expect(
            async () => await getPayload({ salt: val as any })
          ).rejects.toThrow();
        }
      );

      it.each(NAN)(
        'throws an error when the maxSupply is not a number (%s',
        val => {
          expect(
            async () => await getPayload({ maxSupply: val as any })
          ).rejects.toThrow();
        }
      );

      it.each(NAN)(
        'throws an error when the mintPrice is not a number (%s)',
        val => {
          expect(
            async () => await getPayload({ mintPrice: val as any })
          ).rejects.toThrow();
        }
      );

      it.each(NAN)(
        'throws an error when the proposerFee is not a number (%s)',
        val => {
          expect(
            async () => await getPayload({ proposerFee: val as any })
          ).rejects.toThrow();
        }
      );
      ...
      it.skip('throws an error when the proposerFee is out of range', () => {
        expect(
          async () => await getPayload({ proposerFee: '101' })
        ).rejects.toThrow();
        expect(
          async () => await getPayload({ proposerFee: '-5' })
        ).rejects.toThrow();
      });
    });
  });
